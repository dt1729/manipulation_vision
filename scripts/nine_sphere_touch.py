"""
nine_sphere_touch.py

5×5 sphere probe pressed against the Piper gripper finger pad.
Shows multiple touch_grid cells firing simultaneously.

Usage:  python scripts/nine_sphere_touch.py
Output: scripts/nine_sphere_touch.mp4
"""
...
OUTPUT_MP4 = os.path.join(SCRIPT_DIR, "nine_sphere_touch.mp4")
TMP_XML    = os.path.join(os.path.dirname(ROBOT_XML), "_nine_tac.xml")
...
ARM_POSE = {
    ...
    "joint5": 0.4,
    "joint6": 0.0,
    "joint7":  0.015,   # 15 mm opening each side
    "joint8": -0.015,
}

PRESS_AMP  = 0.018   # press 18 mm into pad from start position
PRESS_FREQ = 0.3     # Hz — one slow press cycle every ~3 s
KP         = 100.0
KD         = 15.0

# tactile display constants (matches tactile_video.py)
...
# ── 5×5 sphere probe ─────────────────────────────────────────────────────────
# 25 spheres in a 5×5 grid in the plane perpendicular to contact_axis (world Y).
# Spans ±12 mm in world X and Z — fits within the 30×45 mm finger pad.
SPHERE_R   = 0.005
SPHERE_GAP = 0.012   # center-to-center spacing (4×12mm = 48mm total span)
offsets_xz = [-2*SPHERE_GAP, -SPHERE_GAP, 0.0, SPHERE_GAP, 2*SPHERE_GAP]

# Start probe 8 mm inward from l7g (toward center) for immediate contact
probe_start = l7g + contact_axis * 0.008
probe_body = ET.SubElement(wb, "body", attrib={
    "name": "probe",
    "pos":  f"{probe_start[0]:.5f} {probe_start[1]:.5f} {probe_start[2]:.5f}",
})
probe_body.append(ET.fromstring("<freejoint/>"))

for dx in offsets_xz:
    for dz in offsets_xz:
        ET.SubElement(probe_body, "geom", attrib={
...
for step in range(STEPS):
    ...
    # press probe from probe_start toward link7 pad (–contact_axis direction)
    target_pos = probe_start - contact_axis * (PRESS_AMP * 0.5 * (1 - np.cos(omega * t)))

    probe_pos = d.xpos[probe_bid].copy()
    probe_vel = d.cvel[probe_bid, 3:6].copy()
    force = KP * (target_pos - probe_pos) + KD * (-probe_vel)
    d.xfrc_applied[probe_bid, :3] = force
    ...
    cv2.putText(close_bgr, f"press depth: {press_mm:.1f} mm",
                (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 255, 100), 1)

    three_d = np.vstack([wide_bgr, close_bgr])   # 720×480
